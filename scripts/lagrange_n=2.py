...
linearFrictionCoefficient = np.array([0.1, 0.1])
...
q0 = np.array([np.pi + 1e-10, np.pi+1e-10])
v0 = np.array([0, 0])
...
# Using ffmpeg to save as mp4. ffmpeg must be downloaded and the path below changed to the correct path.
# For some reason, adding the path to the system path does not work for me.
